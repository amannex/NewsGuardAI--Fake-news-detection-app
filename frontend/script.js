document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderHighlights(originalText, explanation) {
        ...
        const wordData = {};
        // Find max absolute weight for normalisation
        let maxWeight = 0;
        ...
        tokens.forEach(token => {
            ...
            if (wordData[lowerToken]) {
                ...
                // Calculate opacity based on weight relative to max weight to visually indicate importance
                const normalizedWeight = Math.min(Math.abs(item.weight) / (maxWeight || 1), 1);
                // Ensure a minimum opacity so it's visible if it's in the explanation
                const opacity = Math.max(0.3, normalizedWeight);
                
                if (item.indicator === 'REAL') {
                    span.classList.add('highlight-real');
                    // dynamically adjust background alpha based on weight
                ...
                span.setAttribute('data-weight', `Weight: ${item.weight.toFixed(4)}`);
